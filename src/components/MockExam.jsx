...
export default function MockExam({ questions, onBack }) {
  ...
  const [timeLimit, setTimeLimit] = useState(120);  // 分。0 = 制限なし

  const [examQuestions, setExamQuestions] = useState([]);
  const [currentIndex, setCurrentIndex]   = useState(0);
  const [selected, setSelected]           = useState(null);
  const [showResult, setShowResult]       = useState(false);
  const [answers, setAnswers]             = useState([]); // null | { chosen, correct }

  const [timeLeft, setTimeLeft]   = useState(0);   // 残り秒
  const [usedTime, setUsedTime]   = useState(0);   // 使用秒
  ...
  function handleSelect(idx) {
    ...
    setAnswers((prev) => {
      const next = [...prev];
      next[currentIndex] = { chosen: idx, correct };
      return next;
    });
    ...
    setHistory((prev) => {
      const prevRec = (prev[histKey] && "attempts" in prev[histKey])
        ? prev[histKey] : { attempts: 0, correctCount: 0 };
      const updated = {
        ...prev,
        [histKey]: {
          attempts:      prevRec.attempts + 1,
          correctCount:  prevRec.correctCount + (correct ? 1 : 0),
        },
      };
  ...
  if (phase === "setup") {
    const questionCount = questions.filter((q) => q.年度 === examYear).length;

    return (
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "24px 16px", fontFamily: "sans-serif" }}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          <button onClick={onBack} style={btnStyle("#e5e7eb", "#374151")}>← 戻る</button>
        ...
        {examYear && (
          <div style={{
            ...
            <InfoItem label="出題数" value={`${questionCount}問`} />
            <InfoItem label="制限時間" value={timeLimit > 0 ? `${timeLimit}分` : "なし"} />
          </div>
        )}

        <button
          onClick={startExam}
          disabled={!examYear || questionCount === 0}
          style={{
            width: "100%", padding: "16px",
            background: examYear && questionCount > 0 ? "#1d4ed8" : "#d1d5db",
  ...
  if (phase === "exam") {
    ...
    const isLastQuestion = currentIndex + 1 >= examQuestions.length;
    ...
    const timerRed = timeLimit > 0 && timeLeft > 0 && timeLeft < 300; // 残り5分で赤
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "16px", fontFamily: "sans-serif" }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 13, color: "#6b7280" }}>
            {currentIndex + 1} / {examQuestions.length}問
          </span>
          {timeLimit > 0 && (
            <span style={{
              fontSize: 20, fontWeight: "bold",
              color: timerRed ? "#dc2626" : "#111827",
              fontVariantNumeric: "tabular-nums",
              background: timerRed ? "#fee2e2" : "transparent",
              padding: timerRed ? "2px 8px" : 0,
              borderRadius: 6,
            }}>
              ⏱ {formatTime(timeLeft)}
            </span>
          ...
          <button
            onClick={() => { if (window.confirm("試験を終了して結果を表示しますか？")) finishExam(); }}
            style={{ fontSize: 12, padding: "4px 10px", borderRadius: 6, border: "1.5px solid #e5e7eb", background: "#fff", cursor: "pointer", color: "#6b7280" }}
          >
            終了
          </button>
        </div>
        ...
        <div style={{
          ...
          marginBottom: 20, lineHeight: 1.7, fontSize: 15, color: "#111827",
        }}>
          {q.問題文}
        </div>
        ...
        {showResult && (
          <div style={{
            ...
            <div style={{ fontSize: 16, fontWeight: "bold", color: isCorrect ? "#15803d" : "#dc2626" }}>
              {isCorrect ? "✅ 正解！" : `❌ 不正解（正解は ${q.正答}）`}
            </div>
            {!isCorrect && q.解説 && (
              <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.7, marginTop: 8 }}>{q.解説}</div>
            )}
          </div>
        )}
  ...
  if (phase === "result") {
    ...
    const subjectResults = SUBJECT_ORDER.map((subj) => {
      const indices = examQuestions
        .map((q, i) => (q.科目 === subj ? i : -1))
        .filter((i) => i !== -1);
    ...
    // 足切り全クリア
    const allSubjectsPassed = subjectResults
    ...
    return (
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "24px 16px", fontFamily: "sans-serif" }}>
        ...
        <div style={{
          ...
          border: `1.5px solid ${overallPassed ? "#bbf7d0" : "#fecaca"}`,
          textAlign: "center",
        }}>
          ...
          <div style={{ fontSize: 42, fontWeight: "bold", marginBottom: 4 }}>
            {totalCorrect}
            <span style={{ fontSize: 20, color: "#6b7280" }}> / {totalQuestions} 点</span>
          </div>
          <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 12 }}>
            正答率 {totalQuestions > 0 ? Math.round(totalCorrect / totalQuestions * 100) : 0}%
            　回答済 {totalAnswered}/{totalQuestions}問
            {usedTime > 0 && `　使用時間 ${formatTime(usedTime)}`}
          </div>
          <div style={{
            display: "inline-block",
            padding: "8px 24px", borderRadius: 99,
            background: overallPassed ? "#dcfce7" : "#fee2e2",
            fontSize: 16, fontWeight: "bold",
            color: overallPassed ? "#15803d" : "#dc2626",
          }}>
            {overallPassed
              ? `✅ 全科目足切りクリア`
              : `❌ 足切り不合格あり`}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
          {subjectResults.map((r) => (
            <div key={r.subj} style={{
              display: "flex", alignItems: "center", gap: 12,
              padding: "12px 16px", borderRadius: 10,
              background: r.passed === false ? "#fef2f2" : r.passed === true ? "#f0fdf4" : "#f9fafb",
              border: `1.5px solid ${r.passed === false ? "#fecaca" : r.passed === true ? "#bbf7d0" : "#e5e7eb"}`,
            }}>
              {/* 科目バッジ */}
              <span style={{
                fontSize: 11, padding: "2px 8px", borderRadius: 99,
                background: r.color.bg, color: r.color.color,
              ...
              <div style={{ flex: 1 }}>
                <div style={{
                  ...
                  <div style={{
                    height: "100%",
                    width: `${Math.round((r.correct / r.total) * 100)}%`,
                    background: r.passed === false ? "#ef4444" : r.passed === true ? "#22c55e" : "#6366f1",
                    borderRadius: 4,
                  }} />
                </div>
              </div>
              ...
              {/* 点数 */}
              <span style={{ fontSize: 15, fontWeight: "bold", minWidth: 60, textAlign: "right" }}>
                {r.correct} / {r.total}
              </span>
          ...
          {examQuestions.map((q, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                background: unanswered ? "#fffbeb" : correct ? "#f0fdf4" : "#fef2f2",
                border: `1px solid ${unanswered ? "#fde68a" : correct ? "#bbf7d0" : "#fecaca"}`,
              }}>
                ...
                <span style={{
                  ...
                  background: subColor.bg, color: subColor.color,
                  fontWeight: "bold", whiteSpace: "nowrap",
                }}>{q.科目?.replace("学科", "").replace("（", "(").replace("）", ")")}</span>
                <span style={{
...
function InfoItem({ label, value }) {
  return (
    <div>
      ...
      <p style={{ fontSize: 16, fontWeight: "bold", margin: 0, color: "#111827" }}>{value}</p>
    </div>
  );
}
...
function btnStyle(bg, color) {
  return {
    ...
    background: bg, color, border: "1.5px solid #e5e7eb",
  };
}
