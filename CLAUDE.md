...
│   └── requirements.txt
└── tools/
    ├── ocr_to_notion.py        # Claude Sonnet OCR → Notion 解説一括登録
    ├── compare_ocr_notion.py   # OCR結果と Notion 解説の照合・修正
...
ANTHROPIC_API_KEY=sk-ant-xxxxx...  # AI機能・OCR 共通
```
...
R1〜R7 合計875問の解説を総合資格の解答解説書から Claude Sonnet OCR で登録済み。
...
総合資格の解答解説書スキャンJPGを Anthropic API（Claude Sonnet）でOCRし、Notionの「解説」フィールドに登録・照合する。

| スクリプト | 役割 |
|-----------|------|
| `tools/ocr_to_notion.py` | 画像OCR → Notion「解説」へ一括登録 |
| `tools/compare_ocr_notion.py` | OCR結果と Notion「解説」を照合し、不一致を修正 |

### sogo_image フォルダ構成
```
tools/sogo_image/
├── R1/
│   ├── 1_keikaku/  page_001.jpg, _ocr_result.txt ...
│   ├── 2_kankyo/
│   ├── 3_hoki/
│   ├── 4_kozo/
│   └── 5_sekko/
...
### 照合・修正（compare_ocr_notion.py）
```powershell
# 照合のみ（キャッシュ利用）
python tools/compare_ocr_notion.py --use-cache --dry-run

# 不一致を Notion に反映
python tools/compare_ocr_notion.py --use-cache --fix

# キャッシュを無視して再OCR
python tools/compare_ocr_notion.py --force-ocr --dry-run

# 特定年度・科目のみ
python tools/compare_ocr_notion.py --year R3 --subject 5_sekko --fix
```

照合結果は `tools/_ocr_notion_compare_report.json` に出力される。一致率 99.5% 未満を不一致と判定。
